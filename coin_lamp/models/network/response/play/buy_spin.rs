...
use super::super::super::super::server::{ContextActionsEnum, LastargsSelectedModeEnum, SpinsBsVEnum, BuyspinModesEnum, StatusCodeEnum, };
...
pub struct Spins {
	...
	pub bs_v: Vec<Vec<SpinsBsVEnum>> /* [[0,0,0],[0,30.0,10.0],[0,30.0,0],[0,20,0]] */,
	...
	pub previous_bs_v: Vec<Vec<SpinsBsVEnum>> /* [[0,20,0],[0,10.0,0],[0,20,0],[0,80,0]] */,
...
impl From<server::Spins> for Spins {
	fn from(obj: server::Spins) -> Self {
		Spins {
			...
			previous_bs_v: obj.previous_bs_v.into_iter().map(|inner_vec| {inner_vec.into_iter().map(Into::into).collect()}).collect(),
